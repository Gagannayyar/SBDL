...
def read_accounts(env, spark, hive_db, enable_hive):
    runtime_filter = ConfigLoader.get_data_filter(env, "account.filter")

    if enable_hive:
        return spark.read("select * from" + hive_db + ".accounts").where(runtime_filter)
        ...
        return spark.read \
            ...
            .load("data/account_samples.csv") \
            .where(runtime_filter)

def read_party(env, spark, hive_db, enable_hive):
    runtime_filter = ConfigLoader.get_data_filter(env, "party.filter")

    if enable_hive:
        return spark.read("select * from" + hive_db + ".parties").where(runtime_filter)
